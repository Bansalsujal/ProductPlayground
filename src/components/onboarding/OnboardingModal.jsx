...
export default function OnboardingModal({ open, user, onComplete }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log('Form submitted with:', { careerStage, industry });
    
    if (!careerStage || !industry) {
    ...
    setIsLoading(true);
    try {
      console.log('Onboarding: Saving user data:', { careerStage, industry });
      ...
      console.log('Onboarding: User stats initialized');
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-6">
            ...
            <Select value={careerStage} onValueChange={setCareerStage} required>
              <SelectTrigger>
                <SelectValue placeholder="Select your current role level">
                  {careerStage ? careerStages.find(s => s.value === careerStage)?.label : null}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="text-xs text-gray-500 mt-2">
            Debug: Career Stage: "{careerStage}", Industry: "{industry}", Disabled: {isLoading || !careerStage || !industry ? 'true' : 'false'}
          </div>
        </form>
      </DialogContent>
    </Dialog>
