...
export default function FeedbackPage() {
  ...
  const session = useMemo(() => {
    ...
    // Get the most recent session (sessions are ordered by -created_date)
    const latestSession = sessions[0];
    console.log('Latest session data:', latestSession);
    console.log('Session feedback:', latestSession.feedback);
    console.log('Session dimension_scores:', latestSession.dimension_scores);
        ...
        <Card className="border-0 shadow-lg">
          ...
          <CardContent className="space-y-4">
            {rubric.map((criterion) => {
              ...
              let dimensionScores = session.dimension_scores;
              if (typeof dimensionScores === 'string') {
                try {
                  dimensionScores = JSON.parse(dimensionScores);
                } catch (e) {
              ...
              console.log('Dimension scores for', criterion, ':', dimensionScores);
              const score = dimensionScores?.[criterion] || 0;
              console.log('Final score for', criterion, ':', score);
          ...
          <Card className="border-0 shadow-lg">
            ...
            <CardContent>
              <p className="text-gray-700 leading-relaxed">
                {(() => {
                  // Handle feedback stored as JSON string
                  if (typeof session.feedback === 'string') {
                    try {
                      const feedbackObj = JSON.parse(session.feedback);
                      return feedbackObj.what_worked_well || "No feedback provided yet.";
                    } catch (e) {
                      return session.what_worked_well || "No feedback provided yet.";
                    }
                  }
                  return session.what_worked_well || session.feedback?.what_worked_well || "No feedback provided yet.";
                })()}
              </p>
            </CardContent>
          </Card>
              ...
              <p className="text-gray-700 leading-relaxed">
                {(() => {
                  ...
                  if (typeof session.feedback === 'string') {
                    try {
                      const feedbackObj = JSON.parse(session.feedback);
                      return feedbackObj.areas_to_improve || "No feedback provided yet.";
                    } catch (e) {
                      return session.areas_to_improve || "No feedback provided yet.";
                    }
                  }
                  return session.areas_to_improve || session.feedback?.areas_to_improve || "No feedback provided yet.";
                })()}
              </p>
            </CardContent>
          </Card>
